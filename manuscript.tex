...
\section{MHLLCEP approximate Riemann solver} \label{sec:HLLCEP}% \footnote{Code site: https://nbviewer.jupyter.org/github/bfly123/Exact-Elastic-plastic-Riemann-Solver-/blob/master/MHLLCEPcopy.ipynb}\label{sec:HLLCEP}
...
\begin{tikzpicture}
    ...
    \node at (1.5,0.28) {$\mathbf{Q}_R$};
\end{tikzpicture}
\caption{MHLLCEP approximate Riemann solver, case 4: with both left and right plastic waves.}
\label{fig:case4}
\end{figure}
...
\begin{figure}
 \begin{tikzpicture}
    \matrix[column sep=0mm, row sep = 0mm]
    {
      ...
      \node  at (-7.5,3) [rotate = 90] {$\rho$ ($\text{kg}/\text{m}^3$)};
      \node at (0.0,3) [rotate = 90]{$u$ ($\text{m}/\text{s}$)};
      \node  at (-7.5,-3)[ rotate = 90]{$p$ ($\text{Pa}$)};
